                            ...
                            <div class="space-y-3 overflow-y-auto max-h-[600px] min-h-0">
                                <div v-for="loc in selected.locations" :key="loc.id" class="fun-card p-4 cursor-pointer hover:bg-blue-50" @click="flyTo(loc)">
                                    ...
                                    <div class="flex gap-2 mt-2" @click.stop>
                                        <button @click="copyCoords(loc)" class="text-xs font-bold text-gray-500 hover:text-black bg-white px-2 py-1 rounded border border-gray-300 shadow-sm active:translate-y-0.5" title="Copy Coords">
                                            <i class="fa-solid fa-location-crosshairs"></i> Copy
                                        </button>
                                        <button @click="openModal('edit-location', loc)" class="text-xs font-bold text-blue-500 hover:text-blue-600 bg-blue-50 px-2 py-1 rounded border border-blue-200 shadow-sm active:translate-y-0.5">
                                            <i class="fa-solid fa-pen"></i> Edit
                                        </button>
                                        <button @click="deleteItem('subject_locations', loc.id)" class="text-xs font-bold text-red-500 hover:text-red-600 bg-red-50 px-2 py-1 rounded border border-red-200 shadow-sm active:translate-y-0.5">
  ...
  <script>
    ...
    createApp({
      setup() {
        ...
        const openModal = (t, item = null) => {
             ...
             if(t === 'edit-location' && item) {
                ...
                nextTick(() => {
                     ...
                     if (pickerMapInstance && item.lat && item.lng) {
        ...
        const submitLocation = async () => { processing.value = true; try { const method = forms.location.id ? 'PATCH' : 'POST'; await api('/location', { method: method, body: JSON.stringify(forms.location) }); viewSubject(selected.value.id); closeModal(); } finally { processing.value = false; } };
...
showMapSidebar, flyToGlobal, flyTo, copyCoords,
fileInput,
