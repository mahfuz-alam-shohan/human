  ...
  async fetch(req, env) {
    ...
    try {
        ...
        if (path === '/api/map-data') {
            ...
            return handleGetMapData(env.DB);
        }
        ...
        if (idMatch) {
            const errTab = requireTab('targets'); if (errTab) return errTab;
            const id = idMatch[1];
            const subject = await env.DB.prepare('SELECT id FROM subjects WHERE id = ?').bind(id).first();
            ...
            return handleGetSubjectFull(env.DB, id, adminRow);
        }

        // Sub-resources
        if (path === '/api/interaction') {
            const err = requirePermission('manageIntel'); if (err) return err;
            return handleInteraction(req, env.DB);
        }
        if (path === '/api/location') {
            const err = requirePermission('manageLocations'); if (err) return err;
            return handleLocation(req, env.DB);
        }
        if (path === '/api/intel') {
            const err = requirePermission('manageIntel'); if (err) return err;
            return handleIntel(req, env.DB);
        }
        if (path === '/api/skills') {
            const err = requirePermission('manageIntel'); if (err) return err;
            return handleSkills(req, env.DB);
        }
        ...
        if (path === '/api/media-link') {
            ...
            return handleMediaLink(req, env.DB);
        }
        ...
        if (path === '/api/delete') {
            ...
            if(safeTables.includes(table)) {
                ...
                if(table === 'subjects') {
                    const subject = await env.DB.prepare('SELECT id FROM subjects WHERE id = ?').bind(id).first();
                    if (!subject) return errorResponse("Subject not found", 404);
                    await env.DB.prepare('UPDATE subjects SET is_archived = 1 WHERE id = ?').bind(id).run();
                } else {
        ...
        if (path === '/api/upload-avatar' || path === '/api/upload-media') {
            ...
            const subject = await env.DB.prepare('SELECT id FROM subjects WHERE id = ?').bind(subjectId).first();
            if(!subject) return errorResponse("Subject not found", 404);
