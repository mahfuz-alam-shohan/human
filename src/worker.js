  ...
  async fetch(req, env) {
    ...
    try {
        ...
        if (path === '/api/location') {
            ...
            if (req.method === 'PATCH') {
                 // Check if location exists and belongs to a subject owned by admin
                 const loc = await env.DB.prepare('SELECT l.id FROM subject_locations l JOIN subjects s ON l.subject_id = s.id WHERE l.id = ? AND s.admin_id = ?').bind(p.id, adminId).first();
